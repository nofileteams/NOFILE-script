...
def handle_start_cmd(left, target):
    # Mac には cmd.exe が無いため bash で実行する
...
def handle_start_pwsh(left, target):
    # Mac版PowerShell (PowerShell Core) の実行ファイル名は 'pwsh'
    # (事前に `brew install --cask powershell` 等でインストールされている必要あり)
...
def handle_pause():
    ...
    if msvcrt or (termios and tty and sys.stdin.isatty()):
        _getch()
    else:
        input()
    print()
...
def handle_addon(left, target):
    ...
    addon_dir = os.path.expanduser('~/Documents/addon')
